...
class GroupsConsumer:
    ...
    async def add_user_to_group(self, data):
        ...
        for uid in user_ids:
            ...
            if user_channel_name:
                await self.channel_layer.group_add(f"chat_group_{group.id}", user_channel_name)
    ...
    async def send_user_groups(self, user_id=None):
        # Enviar los grupos del usuario a su WebSocket
        if user_id is None:
    ...
    async def user_groups(self, event):
        # Manejar el evento que contiene la lista de grupos
        await self.send(text_data=json.dumps({
            'type': 'user_groups',
            'groups': event['groups']
        }))
    ...
    def get_user_groups(self, user_id):
        memberships = GroupMembership.objects.filter(user_id=user_id).select_related('group')
        groups = []
        for membership in memberships:
            group = membership.group
            members = GroupMembership.objects.filter(group=group).select_related('user')
            group_members = [{'id': member.user.id, 'username': member.user.username} for member in members]
            groups.append({
                'id': group.id,
                'name': group.name,
    ...
    def get_group_members(self, group_id):
        ...
        return [{'id': membership.user.id, 'username': membership.user.username} for membership in memberships]
