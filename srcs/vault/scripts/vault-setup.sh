...
ensure_ssl_dir() {
    # Create SSL directory with appropriate permissions
    mkdir -p /tmp/ssl || true
    chmod -R 777 /tmp/ssl || true
    chown -R root:root /tmp/ssl || true
    ...
    log_message "WARNING: SSL certificates not found, will generate them locally"
